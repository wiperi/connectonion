...
Define the platform-neutral hosted-agent session behavior that web and React Native clients share: protocol transport, immutable snapshots, adapter boundaries, signing, persistence, reconnect, and testability.
...
#### Scenario: Signs authenticated payloads
- **WHEN** the client signs CONNECT, INPUT, or ONBOARD_SUBMIT payloads
- **THEN** it SHALL sign canonical JSON with sorted keys and no extra separators, and send the protocol envelope as `payload`, `from`, `signature`, and timestamp data compatible with hosted-agent verification
...
### Requirement: Endpoint resolution and transport fallback
The session core or platform transport composition SHALL resolve hosted-agent endpoints and fall back without requiring UI-specific protocol handling.

#### Scenario: Resolves direct endpoint through relay
- **WHEN** an agent address is published to the relay with reachable HTTP endpoints
- **THEN** the client SHALL verify `/info` against the requested agent address and connect to the matching endpoint's `/ws` WebSocket URL

#### Scenario: Uses relay input fallback
- **WHEN** no reachable direct endpoint is found for a valid remote agent address
- **THEN** the client SHALL connect through the relay input WebSocket and include target address routing fields where required

#### Scenario: Supports local simulator development
- **WHEN** a React Native iOS simulator client requests an address hosted by a local agent with no relay announcement
- **THEN** the client MAY probe local development endpoints such as `http://localhost:8000/info` and connect directly to `ws://localhost:8000/ws` when the address matches
